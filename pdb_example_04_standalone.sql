...
where dbid in (db_id('TwoWestGate'),db_id('SunsetDB'),db_id('VeniceDB'),db_id('WilshireDB'),db_id('TwoWestGlobalDB'),db_id('TwoWestCommonDB'),db_id('TwoWestLogDB')) and cmd = 'AWAITING COMMAND' and spid <> @@spid;
exec(@sql);
end;
go
...
if db_id('TwoWestLogDB') 		is not null drop database TwoWestLogDB;
...
exec PdbcreatePartition 'TwoWestGate','TwoWestLogDB',@DatabaseTypeId=4,@StandAloneSchemaName='Log';
...
create table Log.LogWithdraws
	(	Time				datetime				not null
	,	BranchNumber		nvarchar(16)
	,	Name				nvarchar(128)
	,	City				nvarchar(128)
	,   Address				nvarchar(128)
	,   PostalCode			nvarchar(8)
...
begin
	...
	declare @BranchNumber		nvarchar(16);
	...
	declare @LastName			nvarchar(128);
	declare @NationalNumber		nvarchar(16);
	declare @AccountNumber		nvarchar(32);
	...
	,@FirstName = Users.FirstName,@LastName = Users.LastName,@NationalNumber = Customers.NationalNumber,@AccountNumber = Accounts.AccountNumber
	from PdbCreditCards CreditCards
	join PdbAccounts	Accounts	on CreditCards.BranchId 	= Accounts.BranchId		and CreditCards.AccountId 	= Accounts.Id
	join PdbCustomers	Customers	on Accounts.BranchId		= Customers.BranchId	and Accounts.CustomerId		= Customers.Id
	join PdbUsers		Users		on Customers.BranchId		= Users.BranchId		and Customers.UserId		= Users.Id
	...
	select @BranchNumber = BranchNumber,@Name = Name
	from PdbBranches Branches
	where Branches.Id = @BranchId;

	select @City = City,@Address = Address,@PostalCode = PostalCode	
	from PdbATMs ATMs
	where ATMs.Id = @ATMId;
	...
	begin
		...
		begin
			...
			insert into Log.PdbLogWithdraws (Time,BranchNumber,Name,City,Address,PostalCode,FirstName,LastName,NationalNumber,AccountNumber,CardNumber,CreditBalance,CreditWithdrew)
			values (getdate(),@BranchNumber,@Name,@City,@Address,@PostalCode,@FirstName,@LastName,@NationalNumber,@AccountNumber,@CardNumber,@CreditBalance,@CreditWithdrew);
		end;
	end;
end;
go
